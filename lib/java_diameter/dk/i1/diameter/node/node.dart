...
class Node {
	...
	Map<ConnectionKey,Connection> map_key_conn;
	Set<Peer> persistent_peers;
	Logger logger;
	Object obj_conn_wait;
	NodeImplementation tcp_node;
	NodeImplementation sctp_node;
	 ...
	 */
	void start() throws java.io.IOException, UnsupportedTransportProtocolException {
	 ...
	 */
	void waitForConnectionTimeout(long timeout) throws InterruptedException,ConnectionTimeoutException {
	 ...
	 */
	bool isConnectionKeyValid(ConnectionKey connkey) {
		if(map_key_conn==null)
			return false;
		synchronized(map_key_conn) {
	 ...
	 * connectionKey2Peer()
	 */
	InetAddress connectionKey2InetAddress(ConnectionKey connkey) {
		if(map_key_conn==null)
			return null;
		synchronized(map_key_conn) {
			Connection conn = map_key_conn.get(connkey);
	 ...
	 */
	int nextHopByHopIdentifier(ConnectionKey connkey) throws StaleConnectionException {
		if(map_key_conn==null)
	 ...
	 */
	void sendMessage(Message msg, ConnectionKey connkey) throws StaleConnectionException {
		if(map_key_conn==null)
			throw new StaleConnectionException();
		synchronized(map_key_conn) {
			Connection conn = map_key_conn.get(connkey);
	...
	void sendMessage(Message msg, Connection conn) {
	 ...
	 */
	void initiateConnection(Peer peer, bool persistent) {
		if(persistent) {
	...
	static bool getUseOption(bool setting, String property_name, bool default_setting) {
		if(setting!=null)
			return setting;
	...
	NodeImplementation instantiateNodeImplementation(Level loglevel, String class_name) {
	...
	NodeImplementation loadTransportProtocol(bool setting, String setting_name, bool default_setting,
	...
	void prepare() throws java.io.IOException, UnsupportedTransportProtocolException {
	 ...
	 */
	int calcNextTimeout(NodeImplementation node_impl) {
		long timeout = -1;
		synchronized(map_key_conn) {
			for(Map.Entry<ConnectionKey,Connection> e : map_key_conn.entrySet()) {
				Connection conn = e.getValue();
				if(conn.node_impl!=node_impl) continue;
				bool ready = conn.state==Connection.State.ready;
	...
	void closeConnection(Connection conn, bool reset) {
	...
	void initiateConnectionClose(Connection conn, int why) {
		if(conn.state!=Connection.State.ready)
	...
	bool handleMessage(Message msg, Connection conn) {
	...
	bool isLoopedMessage(Message msg) {
		//6.1.3
	...
	void rejectLoopedRequest(Message msg, Connection conn) {
	 ...
	 */
	static class AVP_VendorSpecificApplicationId extends AVP_Grouped {
		AVP_VendorSpecificApplicationId(AVP a) throws InvalidAVPLengthException, InvalidAVPValueException {
			super(a);
			AVP g[] = queryAVPs();
			if(g.length<2)
				throw new InvalidAVPValueException(a);
			bool found_vendor_id = false;
		...
		AVP_VendorSpecificApplicationId(int vendor_id, int auth_app_id, int acct_app_id) {
		...
		int vendorId() throws InvalidAVPLengthException, InvalidAVPValueException {
			for(AVP a : queryAVPs()) {
		...
		Integer authAppId() throws InvalidAVPLengthException {
			for(AVP a : queryAVPs()) {
		...
		Integer acctAppId() throws InvalidAVPLengthException {
			for(AVP a : queryAVPs()) {
	 ...
	 */
	bool isAllowedApplication(Message msg, Peer peer) {
		try {
			AVP avp;
	...
	void rejectDisallowedRequest(Message msg, Connection conn) {
	...
	void rejectRequest(Message msg, Connection conn, int result_code) {
	...
	bool doElection(String cer_host_id) {
		...
		bool close_other_connection = cmp>0;
		synchronized(map_key_conn) {
			for(Map.Entry<ConnectionKey,Connection> e : map_key_conn.entrySet()) {
				Connection conn = e.getValue();
	...
	bool handleCER(Message msg, Connection conn) {
	...
	bool handleCEA(Message msg, Connection conn) {
	...
	bool handleCEx(Message msg, Connection conn) {
	...
	void addCEStuff(Message msg, Capability capabilities, Connection conn) {
	...
	bool handleDWR(Message msg, Connection conn) {
	...
	bool handleDWA(Message msg, Connection conn) {
		logger.log(Level.FINE,"DWA received from "+conn.host_id);
		conn.timers.markDWA();
		return true;
	}
	bool handleDPR(Message msg, Connection conn) {
	...
	bool handleDPA(Message msg, Connection conn) {
	...
	bool handleUnknownRequest(Message msg, Connection conn) {
	...
	void sendDPR(Connection conn, int why) {
	...
	bool anyOpenConnections(NodeImplementation node_impl) {
		synchronized(map_key_conn) {
			for(Map.Entry<ConnectionKey,Connection> e : map_key_conn.entrySet()) {
				Connection conn = e.getValue();
